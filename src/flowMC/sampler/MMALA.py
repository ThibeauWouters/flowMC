"""Script to implement manifold MALA, see e.g. Riemann manifold Langevin and Hamiltonian Monte Carlo methods by Mark Girolami, Ben Calderhead"""
...
class MMALA(ProposalBase):
    ...
    def __init__(
        self, logpdf: Callable, jit: bool, params: dict, G: Array, use_autotune=False
    ):
        """
        Initialize MMALA. 
        """
        ...
        print("This is G inverse:")
        print(self.G_inv)
        self.use_autotune = use_autotune

    def body(self, carry, this_key):
        print("Compiling MMALA body")
        this_position, dt, data = carry
        dt2 = dt * dt
        this_log_prob, this_d_log = jax.value_and_grad(self.logpdf)(this_position, data)
        proposal = this_position + jnp.dot(dt2, jnp.matmul(self.G_inv, this_d_log)) / 2 # diffusion term
        proposal += jnp.dot(dt, jnp.matmul(self.G_inv_sqrt, jax.random.normal(this_key, shape=this_position.shape))) # random walk term
    ...
    def kernel(
        ...
        # Determine acceptance, (i) evaluate log probabilty
        ratio = logprob[1] - logprob[0]
        # (ii) proposal density (see Girolami, Calderhead)
        ratio -= multivariate_normal.logpdf(
            proposal[0], position + jnp.dot(dt2, jnp.matmul(self.G_inv, d_logprob[0])) / 2, jnp.matmul(self.G_inv, dt2)
        )
        ratio += multivariate_normal.logpdf(
            position, proposal[0] + jnp.dot(dt2, jnp.matmul(self.G_inv, d_logprob[1])) / 2, jnp.matmul(self.G_inv, dt2)
        )
